...
void measureUltrasonicDistances() {
  ...
  if (distance1 <= 15) {  //you can set the distance according to your choice
    setBuzzerPattern(1); //if the condition is set to be ok the it triggers the buzzer sound
...
void SendMessage() {
  ...
  gsmSerial.print("I Am In Problem Plz Help Me. My Location: ");//this message will be the default message send to the caretacker
  p = get_gps();
  gsmSerial.listen();
